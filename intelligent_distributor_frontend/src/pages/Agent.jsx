...
import Beams from '../components/Beams/Beams'; // 檢查路徑是否正確
import { Link } from 'react-router-dom';

// --- 静态资源与常量 ---
// 在实际项目中，这些图标可以放在 public 或 assets 文件夹下
const platformIcons = {
...
function App() {
  // --- 状态管理 (State) ---
  const [view, setView] = useState('upload'); // 'upload' 或 'results'
  ...
  // --- 渲染 (Render) ---
  return (
  <div className="relative w-screen h-screen overflow-hidden bg-gray-900"> {/* 外层容器占满整个屏幕 */}
      <Link
        to="/"  className="absolute top-20 left-40 z-20 font-semibold text-xl text-white tracking-widest animate-fade-in-slide-up">
        THEPAI
      </Link>
    {/* Beams 背景层 */}
    <div className="absolute inset-0 z-0"> {/* 绝对定位覆盖整个父容器，z-index 为 0 */}
      <Beams
        beamWidth={2}
        beamHeight={15}
        beamNumber={12}
    ...
    <div className="relative z-10 flex items-center justify-center w-full h-full p-4 md:p-8"> {/* z-index 为 1，flex 居中 */}
    ...
    <div className="w-11/12 md:w-4/5 lg:w-3/4 h-auto md:h-5/6 flex flex-col bg-black/30 backdrop-blur-lg border border-white/10 shadow-2xl rounded-2xl overflow-hidden">
      ...
      <header className="p-8 border-b border-white/10 shrink-0 flex justify-between items-start">
          ...
          {Object.keys(loginStatuses).map(platform => {
            const { status, message } = loginStatuses[platform];
            const platformStyles = {
              'B站': {
                  base: 'bg-transparent border-sky-500 text-sky-400 hover:bg-sky-500/20',
                  success: 'bg-green-600 border-green-600 text-white'
                },
                '小红书': {
                  base: 'bg-transparent border-red-500 text-red-500 hover:bg-red-500/20',
                  success: 'bg-green-600 border-green-600 text-white'
                },
            };
            const messageStyles = {
              loading: 'text-yellow-400',
              prompt_scan: 'text-green-400',
              error: 'text-red-400',
              idle: ''
            };

            const isSuccess = status === 'prompt_scan';
            ...
            return (
              <div key={platform} className="text-right">
                <button
                  ...
                  className={`font-bold py-2 px-4 rounded-lg border transition-all duration-300 transform hover:scale-105 ${isSuccess ? platformStyles[platform].success : platformStyles[platform].base}`}
                >
                  {isSuccess ? `已请求${platform}` : `登入${platform}`}
                </button>
                <p className={`text-xs mt-1 h-4 ${messageStyles[status]}`}>{message}</p>
              </div>
            );
          })}
        </div>
      ...
      {/* 状态 2: 结果与操作区 */}
      {view === 'results' && (
        <div id="results-state" className="flex flex-col h-full">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 grow">
            ...
            <div className="space-y-4 flex flex-col">
              ...
              <div className="w-full aspect-video rounded-lg bg-black overflow-hidden shrink-0">
                {/* 2. 让 video 元素充满容器，并保持自身比例(object-contain) */}
                <video
                  id="video-preview"
              ...
              <div id="video-info" className="text-sm text-gray-400">
                {currentFile && `文件名: ${currentFile.name}`}
              </div>
            ...
            <div className="space-y-4 flex flex-col">
              <div className="flex justify-between items-center shrink-0">
                ...
                {isAnalyzing && (
                 ...
                 {!isAnalyzing && !analysisError && analysisResult && (
                   <span className="text-sm text-green-400">分析完成！</span>
                 )}
              </div>
              ...
              {!isAnalyzing && !analysisError && analysisResult && (
                  <div className="bg-white/5 border border-white/10 p-4 rounded-xl space-y-2 animate-fade-in shrink-0">
                      ...
                      <div><label className="text-xs font-medium text-gray-400">标签</label>
                          <div className="flex flex-wrap gap-2 mt-1">
                            {Array.isArray(analysisResult.tags) && analysisResult.tags.map(tag => (
                                 <span key={tag} className="bg-gray-700 text-xs font-medium text-indigo-300 px-2.5 py-1 rounded-full">{tag}</span>
                              ))}
                          </div>
                      </div>
                  </div>
              ...
              {/* 推荐列表 */}
              <div id="recommendations-list" className="space-y-3 overflow-y-auto grow">
                  ...
                  return (
                    <div key={rec.platform} className="bg-white/5 border border-white/10 p-4 rounded-xl animate-fade-in space-y-3">
                      ...
                      <div className="text-right">
                        ...
                        <span className={`upload-status text-xs ml-2 ${getStatusMessageClasses()}`}>{uploadStatus.message}</span>
                      </div>
                    </div>
                  );
                })}
